...
- «Заказать продукты» → реально добавляет ингредиенты в `cart_items`
- «Корзина» → показывает корзину и список покупок, «Добавить всё в список» переносит и очищает корзину
...
- `cart_items` — то, что попало в корзину через «Заказать продукты»
- `grocery_items` — итоговый список покупок (need/have/low_stock)
